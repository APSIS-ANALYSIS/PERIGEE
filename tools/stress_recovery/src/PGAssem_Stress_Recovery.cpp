...
{
  const int nlocrow = 9 * pnode_ptr -> get_nlocalnode();

  // Allocate the sparse matrix K
  MatCreateAIJ(PETSC_COMM_WORLD, nlocrow, nlocrow, PETSC_DETERMINE,
      PETSC_DETERMINE, 9*in_nz_estimate, NULL, 9*in_nz_estimate, NULL, &K);
    ...
    const APart_Node * const &pnode_ptr )
{
  ...
  PetscInt * row_index = new PetscInt [nLocBas * 9];
  ...
  for(int ee=0; ee<nElem; ++ee)
  {
    for(int ii=0; ii<nLocBas; ++ii)
    {
      ...
      for(int mm=0; mm<9; ++mm)
        row_index[9 * ii + mm] = 9 * pnode_ptr->get_local_to_global(loc_index) + mm;
    }

    MatSetValues(K, 9 * nLocBas, row_index, 9 * nLocBas, row_index, lassem_ptr->Tangent, ADD_VALUES);
  }
