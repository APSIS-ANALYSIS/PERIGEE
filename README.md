...
## Simulation Samples
The pulmonary model is built from a healthy 20 month old male, consisting of 772 outlets. The simulation is performed with a boundary layer mesh with 26 million linear elements. The model is prepared by M. Dong and the mesh is prepared by W. Yang. Click the image for a youtube video.
...
A fluid-structure interaction simulation of a pulmonary model using the unified continuum and variational multiscale formulation. The model and mesh are prepared by W. Yang. The solid model is *fully incompressible* and is numerically modeled via the residual-based variational multiscale formulation.
