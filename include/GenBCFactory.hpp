...
class GenBCFactory
{
  public:
    static std::unique_ptr<IGenBC> createGenBC(const std::string &lpn_file,
        const double &initial_step = 0.1, const int &initial_index = 0,
        const double &initial_time = 0.0)
    {
      ...
      switch (file_type)
      {
        ...
        case 2:
          return SYS_T::make_unique<GenBC_RCR>(lpn_file, 1000, initial_step);
        case 3:
          return SYS_T::make_unique<GenBC_Inductance>(lpn_file);
        case 4:
          return SYS_T::make_unique<GenBC_Coronary>(lpn_file, 1000, initial_step, initial_index);
        case 5:
