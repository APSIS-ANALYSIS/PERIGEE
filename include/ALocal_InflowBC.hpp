...
class ALocal_InflowBC
{
  public:
    ...
    // ------------------------------------------------------------------------
    // access an element's IEN array by indexing the local_sur_ien array
    // 0 <= ii < cell_nLocBas[nbc_id] x num_local_cell[nbc_id]
    // Note: make sure num_local_cell[nbc_id] > 0 before using this get function
    // ------------------------------------------------------------------------
    virtual int get_local_sur_ien( const int &nbc_id, const int &ii ) const
    {return local_sur_ien[nbc_id][ii];}
    ...
    // Note: make sure num_local_cell[nbc_id] > 0 before using this get function
    // ------------------------------------------------------------------------
    virtual int get_local_sur_ien( const int &nbc_id, const int &ee, 
        const int &ii ) const
    { return local_sur_ien[nbc_id][ee * cell_nLocBas[nbc_id] + ii]; }
