...
namespace LoadData
{
  ...
  // --------------------------------------------------------------------------
  inline void rescale_dot_inflow_value( const double &stime,
      const ALocal_InflowBC * const &infbc,
      const IFlowRate * const &flrate,
      const PDNSolution * const &sol_base,
      PDNSolution * const &dot_sol )
  {
    ...
    for(int nbc_id=0; nbc_id<num_nbc; ++nbc_id)
    {
      ...
      const double factor  = flrate -> get_dot_flow_rate( nbc_id, stime );
      const double std_dev = flrate -> get_flow_TI_std_dev( nbc_id );
      ...
      for(int ii=0; ii<numnode; ++ii)
      {
        ...
        VecSetValues(dot_sol->solution, 3, base_idx, vals, INSERT_VALUES);
      }
    }
  ...
  // -------------------------------------------------------------------------- 
  inline void rescale_dot_inflow_velo( const double &stime,
      const ALocal_InflowBC * const &infnbc,
      const IFlowRate * const &flrate,
      const PDNSolution * const &sol_base,
