...
std::vector<double> get_coords(const int &tt);
...
int main( int argc, char * argv[] )
{
  ...
  std::vector<int> global_node_idx (v_nFunc, 0);
  PERIGEE_OMP_PARALLEL_FOR
  for(int ii=0; ii<v_nFunc; ++ii)
    global_node_idx[ii] = ii;
  ...
  for(int time = time_start; time <= time_end; time += time_step)
  {
    ...
    pp_inlet += inv_T * sol[global_node_idx[552312] * dof + 0];
    pp_outlet += inv_T * sol[global_node_idx[2250570] * dof + 0];
  ...
  std::cout<<"  averaged pressure on inlet = "<<pp_inlet<<std::endl;
  std::cout<<"  averaged pressure on outlet ="<<pp_outlet<<std::endl;
...
std::vector<double> get_coords(const int &tt)
{
  std::string fileName = get_ansys_result_name("FFF-1-01000-1-", tt, ".cas.h5");
