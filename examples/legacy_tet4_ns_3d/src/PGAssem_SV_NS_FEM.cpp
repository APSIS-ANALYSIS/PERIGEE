#include "PGAssem_SV_NS_FEM.hpp"

PGAssem_SV_NS_FEM::PGAssem_SV_NS_FEM(
    ...
    ALocal_EBC const * const &part_ebc )
{
  ...
  const int empirical_neibor_number = 50;
  //Get_dnz_onz( nlocalnode,  empirical_neibor_number,
  //    part_nbc, dnnz, onnz );

  MatCreateAIJ(PETSC_COMM_WORLD, nlocrow, nlocrow, PETSC_DETERMINE,
      PETSC_DETERMINE, empirical_neibor_number*dof, NULL, 
      empirical_neibor_number*dof, NULL, &K);
  ...
  for(int ebc_id=0; ebc_id < num_ebc; ++ebc_id)
    SYS_T::print_fatal_if(snLocBas != part_ebc->get_cell_nLocBas(ebc_id),
        "Error: in PGAssem_SV_NS_FEM, snLocBas has to be uniform. \n");
...
void PGAssem_SV_NS_FEM::Get_dnz_onz( const int &nElem,
...
void PGAssem_SV_NS_FEM::Get_dnz_onz( const int &nlocnode,
...
void PGAssem_SV_NS_FEM::Assem_nonzero_estimate(
...
void PGAssem_SV_NS_FEM::EssBC_KG( 
...
void PGAssem_SV_NS_FEM::EssBC_G( 
    const ALocal_NodalBC * const &nbc_part, const int &field )
{
  const int local_dir = nbc_part->get_Num_LD(field);
  const int local_sla = nbc_part->get_Num_LPS(field);
...
void PGAssem_SV_NS_FEM::NatBC_G( const double &curr_time, const double &dt,
    IPLocAssem * const &lassem_ptr,
...
void PGAssem_SV_NS_FEM::Assem_mass_residual(
...
void PGAssem_SV_NS_FEM::Assem_residual(
    const PDNSolution * const &sol_a,
...
void PGAssem_SV_NS_FEM::Assem_tangent_residual(
    const PDNSolution * const &sol_a,
    const PDNSolution * const &sol_b,
    const double &curr_time,
    const double &dt,
...
double PGAssem_SV_NS_FEM::Assem_outsurface_flowrate(
    IPLocAssem * const &lassem_ptr,
...
{
  const int num_sur_ele = ebc_part -> get_num_local_cell(ebc_id);
  double ele_sum = 0.0;

  for(int ee=0; ee<num_sur_ele; ++ee)
  {
    ebc_part -> get_SIEN( ebc_id, ee, LSIEN );
    ebc_part -> get_ctrlPts_xyz( ebc_id, ee, sctrl_x, sctrl_y, sctrl_z );
    ...
    ele_sum += lassem_ptr-> get_flowrate( local_bs, ele_s, sctrl_x,
        sctrl_y, sctrl_z, quad_s );
  }

  double all_sum = 0.0;
  MPI_Allreduce(&ele_sum, &all_sum, 1, MPI_DOUBLE, MPI_SUM, PETSC_COMM_WORLD);
  return all_sum;
}
