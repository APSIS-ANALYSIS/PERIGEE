// ==================================================================
// seg_diagscale_tissue_driver.cpp
// This solver uses diagonal scaling for the matrices.
// and reads in the preprocessor info from a Gmsh geometry output.
// 
// This solver is for hyperelastic soft tissue model from  Gasser-
// Ogden-Holzapfel Journal of Royal Society Interface 2006.
//
// Author: Ju Liu
// Date: April 20 2018
// ==================================================================
...
int main(int argc, char *argv[])
{
  ...
  SYS_T::GetOptionReal("-ang", angle);
  SYS_T::GetOptionReal("-kd", kdisp);
  ...
  SYS_T::cmdPrint("-ang:", angle);
  SYS_T::cmdPrint("-kd:", kdisp);
  ...
  const double mat_fangle = angle; //90.0 - 49.98;
  const double mat_rho = 1.0;
  //const double mat_nu = 0.4999;
  const double mat_mu = 7.64e4;
  //const double mat_E = 2.0 * (1.0 + mat_nu) * mat_mu;
  const double mat_k_disper = kdisp;

  //IMaterialModel * matmodel = new MaterialModel_GOH06_ST91_Mixed(
  //    mat_rho, mat_E, mat_nu, mat_fangle, 0.0, (-1.0) * mat_fangle, 0.0, 
  //    9.966e7, 524.6, mat_k_disper );
  
  IMaterialModel * matmodel = new MaterialModel_GOH06_Incompressible_Mixed(
      mat_rho, mat_mu, 90,  mat_fangle, 90.0, (-1.0) * mat_fangle, 
      9.966e6, 524.6, mat_k_disper );
  ...
  PLinear_Solver_DiagScale * lsolver = new PLinear_Solver_DiagScale(
      ll_rtol, ll_atol, ll_dtol, ll_maxits, gloAssem_ptr );
