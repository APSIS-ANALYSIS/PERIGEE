...
int main( int argc, char * argv[] )
{
  ...
  // ----------------------------------------------------------------
  IVisDataPrep * visprep = new VisDataPrep_Mixed_U_Aniso_Hyperelastic_3D( isRef );
  visprep->print_info();
  ...
  for(int time = time_start; time<=time_end; time+= time_step)
  {
    ...
    if( isRef )
      ...
      vtk_w_s->writeOutput_cur_anisotropic( fNode, locIEN, locElem,
          visprep, matmodel, element, quad, pointArrays,
          rank, size, time * dt, out_bname, name_to_write, isXML );
  }
