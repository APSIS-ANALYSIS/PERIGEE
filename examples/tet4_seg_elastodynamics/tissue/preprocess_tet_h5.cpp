...
int main( int argc, char * argv[] )
{
  ...
  // Disp_x is fixed on bot and top disallow deformation 
  std::vector<int> vnbclist; vnbclist.clear();
  vnbclist.push_back(1); // x = 0
  vnbclist.push_back(0); // x = 1
  NBC_list[1] = new NodalBC_3D( vnbclist, bcpt_2D, nFunc, 1 );

  // Disp_y is fixed on top and left 
  vnbclist.clear();
  vnbclist.push_back(0); // x = 1
  vnbclist.push_back(5); // y = 0
  NBC_list[2] = new NodalBC_3D( vnbclist, bcpt_2D, nFunc);

  // Disp_z is fixed on top and left 
  vnbclist.clear();
  vnbclist.push_back(0); // x = 1
  vnbclist.push_back(2); // z = 0
  NBC_list[3] = new NodalBC_3D( vnbclist, bcpt_2D, nFunc );
  
  // Element BC
  std::vector<int> ebclist; ebclist.clear();
  ebclist.push_back( 0 ); // Top face applies traction force
