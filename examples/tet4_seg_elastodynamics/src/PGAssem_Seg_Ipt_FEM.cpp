...
PGAssem_Seg_Ipt_FEM::PGAssem_Seg_Ipt_FEM( 
    ...
    ALocal_EBC const * const &part_ebc )
{
  ...
  SYS_T::print_fatal_if(dof_mat != part_nbc->get_dof_LID(),
      "PGAssem_Seg_Ipt_FEM::dof_mat != part_nbc->get_dof_LID(). \n");
  ...
  for(int ebc_id=0; ebc_id < num_ebc; ++ebc_id){
    SYS_T::print_fatal_if(snLocBas != part_ebc->get_cell_nLocBas(ebc_id),
        "Error: in PGAssem_Seg_Ipt_FEM, snLocBas has to be uniform. \n");
  }
  ...
  const int emp_neib_node = 120;

  MatCreateAIJ(PETSC_COMM_WORLD, nlocrow, nlocrow, PETSC_DETERMINE,
      PETSC_DETERMINE, emp_neib_node*dof_mat, NULL, emp_neib_node*dof_mat, NULL, &K);
...
void PGAssem_Seg_Ipt_FEM::Get_dnz_onz( const int &nlocnode,
...
void PGAssem_Seg_Ipt_FEM::EssBC_KG( 
...
void PGAssem_Seg_Ipt_FEM::EssBC_G( 
    const ALocal_NodalBC * const &nbc_part, const int &field )
{
  const int local_dir = nbc_part->get_Num_LD(field);
  const int local_sla = nbc_part->get_Num_LPS(field);
...
void PGAssem_Seg_Ipt_FEM::Assem_nonzero_estimate(
...
void PGAssem_Seg_Ipt_FEM::Assem_mass_residual(
...
void PGAssem_Seg_Ipt_FEM::Assem_residual(
    const PDNSolution * const &sol_a,
    ...
    const ALocal_EBC * const &ebc_part )
{
  ...
  NatBC_G(curr_time, dt, lassem_ptr, elements, dof_mat*snLocBas, quad_s,
...
void PGAssem_Seg_Ipt_FEM::Assem_tangent_residual(
    const PDNSolution * const &sol_a,
    const PDNSolution * const &sol_b,
    const double &curr_time,
    const double &dt,
...
void PGAssem_Seg_Ipt_FEM::NatBC_G( const double &curr_time, const double &dt,
    IPLocAssem * const &lassem_ptr,
  ...
  for(int ebc_id = 0; ebc_id < num_ebc; ++ebc_id)
  {
    ...
    for(int ee=0; ee<num_sele; ++ee)
    {
      ...
      lassem_ptr->Assem_Residual_EBC( ebc_id, curr_time, dt,
          int_x, int_y, int_z, local_as, local_bs, element_s, 
