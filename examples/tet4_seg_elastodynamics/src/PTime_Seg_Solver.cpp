...
void PTime_Seg_Solver::TM_Seg_GenAlpha( const bool &restart_init_assembly_flag,
    ...
    PNonlinear_Seg_Solver * const &nsolver_ptr ) const
{
  ...
  while( time_info->get_time() < final_time )
  {
    ...
    nsolver_ptr->GenAlpha_Seg_solve_2( renew_flag, time_info->get_time(),
...
void PTime_Seg_Solver::TM_Seg_GenAlpha_DiagScale( 
    ...
    PNonlinear_Seg_Solver * const &nsolver_ptr ) const
{
  ...
  PetscClassId classid_tsolver;
  PetscClassIdRegister("tsolver", &classid_tsolver);
  PetscLogEventRegister("tsolver", classid_tsolver, &tsolver_event);
  ...
  while( time_info->get_time() < final_time )
  {
    ...
    nsolver_ptr->GenAlpha_Seg_solve_DiagScale( renew_flag, time_info->get_time(),
        time_info->get_step(), pre_velo, pre_disp, tmga_ptr,
        alelem_ptr, lien_ptr, anode_ptr, feanode_ptr, nbc_part, ebc_part,
        bc_mat, elementv, elements, quad_v, quad_s, lassem_ptr,
        gassem_ptr, lsolver_ptr, cur_velo, cur_disp, conv_flag, nl_counter );
