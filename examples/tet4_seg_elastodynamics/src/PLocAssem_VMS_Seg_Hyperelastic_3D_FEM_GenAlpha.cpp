...
PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha(
  ...
  nqp(in_nqp), snLocBas(in_snlocbas)
{
  ...
  flist[0] = &PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::get_top_H;
...
PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::~PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha()
{
  delete [] Tangent; delete [] Residual;
  Tangent = NULL; Residual = NULL;
...
void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::print_info() const
{
...
void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::Zero_Tangent_Residual()
{
...
void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::Zero_Residual()
{
  for(int ii=0; ii<vec_size; ++ii) Residual[ii] = 0.0;
}


void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::Assem_Estimate()
{
  for(int ii=0; ii<vec_size * vec_size; ++ii) Tangent[ii] = 1.0;
}


void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::get_tau( 
...
void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::Assem_Residual(
    ...
    const IQuadPts * const &quad )
{
  ...
  for(qua=0; qua < nqp; ++qua)
  {
    ...
    for(ii=0; ii<nLocBas; ++ii)
    {
      ...
      ux_xx += disp[ii7+0] * d2R_dxx[ii];
      uy_xx += disp[ii7+1] * d2R_dxx[ii];
      uz_xx += disp[ii7+2] * d2R_dxx[ii];

      ux_yy += disp[ii7+0] * d2R_dyy[ii];
      uy_yy += disp[ii7+1] * d2R_dyy[ii];
      uz_yy += disp[ii7+2] * d2R_dyy[ii];

      ux_zz += disp[ii7+0] * d2R_dzz[ii];
      uy_zz += disp[ii7+1] * d2R_dzz[ii];
      uz_zz += disp[ii7+2] * d2R_dzz[ii];

      ux_xy += disp[ii7+0] * d2R_dxy[ii];
      uy_xy += disp[ii7+1] * d2R_dxy[ii];
      uz_xy += disp[ii7+2] * d2R_dxy[ii];

      ux_xz += disp[ii7+0] * d2R_dxz[ii];
      uy_xz += disp[ii7+1] * d2R_dxz[ii];
      uz_xz += disp[ii7+2] * d2R_dxz[ii];

      ux_yz += disp[ii7+0] * d2R_dyz[ii];
      uy_yz += disp[ii7+1] * d2R_dyz[ii];
      uz_yz += disp[ii7+2] * d2R_dyz[ii];
...
void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::Assem_Tangent_Residual(
    const double &time, const double &dt,
    const double * const &velo,
    const double * const &disp,
    FEAElement * const &element,
    ...
    const IQuadPts * const &quad )
{
  ...
  int ii, jj, kk, ll, qua, A, B, index, ii7;
  double p, p_t, p_x, p_y, p_z;
  double ux_t, uy_t, uz_t, vx, vy, vz;
  double vx_t, vy_t, vz_t;
  double ux_x, uy_x, uz_x, ux_y, uy_y, uz_y, ux_z, uy_z, uz_z;
...
void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::Assem_Mass_Residual(
    const double * const &disp,
    FEAElement * const &element,
    const double * const &eleCtrlPts_x,
    const double * const &eleCtrlPts_y,
...
void PLocAssem_VMS_Seg_Hyperelastic_3D_FEM_GenAlpha::Assem_Residual_EBC(
    const int &ebc_id,
    const double &time, const double &dt,
