...
int main( int argc, char * argv[] )
{
  ...
  for(unsigned int ii=0; ii<phy_tag.size(); ++ii)
  {
    ...
    if(!isRightTag) SYS_T::print_fatal("Error: FSI problem, the physical tag for element is wrong\n");
  }
  ...
  for( int ii=0; ii<3; ++ii )
    NBC_list_v[ii] = new NodalBC_3D_FSI( geo_f_file, geo_s_file_in, sur_f_file_wall, // need modification
