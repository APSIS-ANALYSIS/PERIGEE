  ...
  nqp(in_nqp), snLocBas(in_snlocbas), CI(36.0), CT(4.0)
{
  ...
  else flist = new locassem_tet4_ale_vms_ns_funs [num_ebc_fun];
  
  // Users have to specify the functions for ebc integration in correct order
  //flist[0] = &PLocAssem_Tet4_ALE_VMS_NS_3D_GenAlpha::get_H1;
  
  // Print local assembly on screen
  print_info();
}
...
void PLocAssem_Tet4_ALE_VMS_NS_3D_GenAlpha::print_info() const
{
  ...
  PetscPrintf(PETSC_COMM_WORLD, "  Spatial: ALE-VMS \n");
  ...
  PetscPrintf(PETSC_COMM_WORLD, "  Pressure is evaluated at n+alpha_f rather than n+1. \n");
  PetscPrintf(PETSC_COMM_WORLD, "  Input solution vector should have 7 dofs including the mesh motion information in the first three slots. \n");
  PetscPrintf(PETSC_COMM_WORLD, "----------------------------------------------------------- \n");
}


void PLocAssem_Tet4_ALE_VMS_NS_3D_GenAlpha::get_currPts(
...
void PLocAssem_Tet4_ALE_VMS_NS_3D_GenAlpha::get_currBCPts(
    const double * const &ept_x, const double * const &ept_y,
    const double * const &ept_z, const double * const &sol )
{
...
void PLocAssem_Tet4_ALE_VMS_NS_3D_GenAlpha::get_DC( double &dc_tau, 
    ...
    const double &u, const double &v, const double &w ) const
{
  ...
  if(dc_tau > 1.0e-15) dc_tau = std::pow(dc_tau, -0.5);
}
    ...
    const IQuadPts * const &quad )
{
  ...
  for(qua=0; qua<nqp; ++qua)
  {
    ...
    rx = u_t + u_x * cu + u_y * cv + u_z * cw + p_x - f1;
    ry = v_t + v_x * cu + v_y * cv + v_z * cw + p_y - f2;
    rz = w_t + w_x * cu + w_y * cv + w_z * cw + p_z - f3;
    ...
    for(A=0; A<nLocBas; ++A)
    {
      ...
      Residual[4*A+1] += gwts * (NA * u_t
          + NA * (cu * u_x + cv * u_y + cw * u_z)
          - NA_x * p
          + NA_x * two_nu * u_x
          + NA_y * nu * (u_y + v_x)
          + NA_z * nu * (u_z + w_x)
          + velo_dot_gradR * tau_m * rx
          - NA * tau_m * r_dot_gradu
          + NA_x * tau_c * div_vel
      ...
      Residual[4*A+2] += gwts * (NA * v_t
          + NA * (cu * v_x + cv * v_y + cw * v_z)
          - NA_y * p
          + NA_x * nu * (u_y + v_x)
          + NA_y * two_nu * v_y
          + NA_z * nu * (v_z + w_y)
          + velo_dot_gradR * tau_m * ry
          - NA * tau_m * r_dot_gradv
          + NA_y * tau_c * div_vel
      ...
      Residual[4*A+3] += gwts * (NA * w_t
          + NA * (cu * w_x + cv * w_y + cw * w_z)
          - NA_z * p
          + NA_x * nu * (u_z + w_x)
          + NA_y * nu * (w_y + v_z)
          + NA_z * two_nu * w_z
          + velo_dot_gradR * tau_m * rz
          - NA * tau_m * r_dot_gradw
          + NA_z * tau_c * div_vel
    ...
    for(A=0; A<nLocBas; ++A)
    {
      ...
      for(B=0; B<nLocBas; ++B)
      {
        ...
        Sub_Tan[5][index] += gwts * ( 
            alpha_m * ( NANB + ( velo_dot_gradR + NA_x * u ) * tau_m * NB
    ...
    const IQuadPts * const &quad )
{
  ...
  for(qua=0; qua<nqp; ++qua)
  {
    ...
    for(A=0; A<nLocBas; ++A)
    {
      ...
      Residual[4*A+1] += gwts * ( NA * (u*u_x + v*v_y + w*u_z) 
          - NA_x * p
          + two_nu * NA_x * u_x
          + nu * NA_y * (u_y + v_x)
          + nu * NA_z * (u_z + w_x)
