    ...
    bool &conv_flag, int &nl_counter ) const
{
  ...
  do
  {
    //nsoltimer->Reset(); nsoltimer->Start();
    lsolver_ptr->Solve( gassem_ptr->G, dot_P_V_step );
    //nsoltimer->Stop();
    //PetscPrintf(PETSC_COMM_WORLD, ", S %e sec ", nsoltimer->get_sec());
    ...
    nsoltimer->Stop();
    //PetscPrintf(PETSC_COMM_WORLD, " A %e sec,", nsoltimer->get_sec() );
...
void PNonlinear_Seg_Solver::GenAlpha_Seg_solve_FSI_ElasticStiffen(
    const bool &new_tangent_flag,
    const double &curr_time,
    const double &dt,
    const PDNSolution * const &pre_dot_sol,
