...
class PNonlinear_Seg_Solver
{
  public:
    ...
    // --------------------------------------------------------------
    // GenAlpha_Seg_solve_FSI_ElasticStiffen:
    // This is the coupled FSI solver.
    // 
    // The solid and fluid are discretized by VMS and generalized-alpha.
    //
    // The mesh is updated by solving a pseudo-elastotatic problem with
    // adaptively chosen element stiffening effect.
    // --------------------------------------------------------------
    void GenAlpha_Seg_solve_FSI_ElasticStiffen(
        const bool &new_tangent_flag,
        const double &curr_time,
        const double &dt,
        const PDNSolution * const &pre_dot_sol,
