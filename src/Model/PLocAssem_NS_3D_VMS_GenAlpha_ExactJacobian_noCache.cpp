#include "PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache.hpp"

PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache(
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::print_info() const
{
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::Zero_Tangent_Residual()
{
  for(int ii=0; ii<vec_size; ++ii)
    Residual[ii] = 0.0;
  for(int ii=0; ii<vec_size*vec_size; ++ii)
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::Zero_Residual()
{
  for(int ii=0; ii<vec_size; ++ii)
    Residual[ii] = 0.0;
}
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::Assem_Estimate()
{
  for(int ii=0; ii<vec_size*vec_size; ++ii)
    Tangent[ii] = 1.0;
}
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::get_tau(
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::Assem_Residual(
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::Assem_Tangent_Residual(
    const double &time, const double &dt,
    const double * const &velo,
    const double * const &disp,
    const int &eindex,
  ...
  for(qua=0; qua<nqp; ++qua)
  {
    ...
    for(A=0; A<nLocBas; ++A)
    {
      ...
      for(B=0; B<nLocBas; ++B)
      {
        ...
        Sub_Tan_f[0][index] += gwts * ( (-1.0) * velo_dot_gradR * NB - u * NAxNB
        ...
        Sub_Tan_f[1][index] += gwts * ( (-1.0) * u * NAyNB + nu * NAyNBx + tau_m * rx * NAyNB
        ...
        Sub_Tan_f[2][index] += gwts * ( (-1.0) * u * NAzNB + nu * NAzNBx + tau_m * rx * NAzNB
        ...
        Sub_Tan_f[4][index] += gwts * ( (-1.0) * v * NAxNB + nu * NAxNBy
            + tau_m * ry * NAxNB 
        ...
        Sub_Tan_f[5][index] += gwts * ( (-1.0) * velo_dot_gradR * NB - v * NAyNB
        ...
        Sub_Tan_f[6][index] += gwts * ( (-1.0) * v * NAzNB + nu * NAzNBy
            + tau_m * ry * NAzNB
        ...
        Sub_Tan_f[8][index] += gwts * ( (-1.0) * w * NAxNB + nu * NAxNBz
            + tau_m * rz * NAxNB 
        ...
        Sub_Tan_f[9][index] += gwts * ( (-1.0) * w * NAyNB + nu * NAyNBz
            + tau_m * rz * NAyNB
        ...
        Sub_Tan_f[10][index] += gwts * ( (-1.0) * velo_dot_gradR * NB - w * NAzNB
...
void PLocAssem_NS_3D_VMS_GenAlpha_ExactJacobian_noCache::Assem_Mass_Residual(
    const double * const &disp,
    const int &eindex,
    const double &hx, const double &hy, const double &hz,
    const BernsteinBasis_Array * const &bs,
