    ...
    const std::string &filename ) : thred_time( in_thred_time )
{
  ...
  if( bc_type.compare("Inflow") == 0 || bc_type.compare("INFLOW") == 0 )
  {
    ...
    SYS_T::print_fatal("CVFlowRate_Cosine2Steady Error: inlet BC type in %s should be Inflow.\n", filename.c_str());
  ...
  for(int nbc_id=0; nbc_id<num_nbc; ++nbc_id)
  {
    while( std::getline(reader, sline) )
    {
      ...
      if( sline[0] !='#' && !sline.empty() )
      {
        ...
        if( face_id != nbc_id ) SYS_T::print_fatal("CVFlowRate_Cosine2Steady Error: nbc in %s should be listed in ascending order.\n", filename.c_str());
    ...
    if( std::abs(2.0 * MATH_T::PI / period[nbc_id] - w[nbc_id] ) >= 0.01 )
      SYS_T::commPrint( "\nCVFlowRate_Cosine2Steady WARNING: nbc_id %d incompatible period and w, \n2xpi/period = %e and w = %e.\n", nbc_id, 2.0*MATH_T::PI/period[nbc_id], w[nbc_id] );
    ...
    if( static_cast<int>(coef_a[nbc_id].size()) != num_of_mode[nbc_id]+1 )
      SYS_T::print_fatal("CVFlowRate_Cosine2Steady Error: nbc_id %d a-coefficients in %s incompatible with the given number of modes.\n", nbc_id, filename.c_str());
    ...
    if( static_cast<int>(coef_b[nbc_id].size()) != num_of_mode[nbc_id]+1 )
      SYS_T::print_fatal("CVFlowRate_Cosine2Steady Error: nbc_id %d b-coefficients in %s incompatible with the given number of modes.\n", nbc_id, filename.c_str());
  }
...
double CVFlowRate_Cosine2Steady::get_flow_rate( const int &nbc_id,
...
void CVFlowRate_Cosine2Steady::print_info() const
{
  SYS_T::print_sep_line();
  SYS_T::commPrint("     CVFlowRate_Cosine2Steady:\n");
