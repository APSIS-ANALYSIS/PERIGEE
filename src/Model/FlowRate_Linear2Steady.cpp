    ...
    while( std::getline(reader, sline) )
    {
      ...
      if( sline[0] !='#' && !sline.empty() )
      {
        ...
        if(!(sstrm >> num_of_mode[nbc_id]))
          SYS_T::print_fatal("FlowRate_Linear2Steady Error: num_of_mode of nbc %d is undefined!\n", nbc_id);
        
        if(!(sstrm >> w[nbc_id]))
          SYS_T::print_fatal("FlowRate_Linear2Steady Error: w of nbc %d is undefined!\n", nbc_id);
        
        if(!(sstrm >> period[nbc_id]))
          SYS_T::print_fatal("FlowRate_Linear2Steady Error: period of nbc %d is undefined!\n", nbc_id);

        if(!(sstrm >> thred_time[nbc_id]))
          SYS_T::print_fatal("FlowRate_Linear2Steady Error: thred_time of nbc %d is undefined!\n", nbc_id);
