...
MaterialModel_GOH11_Incompressible_Mixed::MaterialModel_GOH11_Incompressible_Mixed(
    ...
    const double &in_fk1, const double &in_fkd )
...
: pt33( 1.0 / 3.0 ), mpt67( -2.0 * pt33 ), pi( MATH_T::PI )
{
  ...
  SYS_T::print_fatal_if( h5r->read_string("/", "model_name") != get_model_name(),
     "Error: MaterialModel_GOH11_Incompressible_Mixed constructor does not match h5 file.\n" );
...
void MaterialModel_GOH11_Incompressible_Mixed::print_info() const
{
  SYS_T::commPrint("\t  MaterialModel_GOH11_Incompressible_Mixed: \n");
...
void MaterialModel_GOH11_Incompressible_Mixed::write_hdf5( const char * const &fname ) const
{
...
void MaterialModel_GOH11_Incompressible_Mixed::get_PK( 
    const Tensor2_3D &F, Tensor2_3D &P, Tensor2_3D &S ) const
{
  ...
  S.AXPY(2.0 * detFm0d67 * fk1, PxH1);
  S.AXPY(2.0 * detFm0d67 * fk1, PxH2);

  // 1st PK
  P.MatMult(F,S);
}

void MaterialModel_GOH11_Incompressible_Mixed::get_PK_Stiffness(
    const Tensor2_3D &F, Tensor2_3D &P, Tensor2_3D &S, Tensor4_3D &CC ) const
{
  ...
  CC.add_OutProduct(val1, PxH1, PxH1);
  CC.add_OutProduct(val1, PxH2, PxH2);

  const double val2 = 2.0 * pt33 * detFm0d67 * ( mu * trC 
      + 2.0 * fk1 * fE1 * ( fkd * trC + (1.0-3.0*fkd)*a1Ca1 )
      + 2.0 * fk1 * fE2 * ( fkd * trC + (1.0-3.0*fkd)*a2Ca2 ) );
...
double MaterialModel_GOH11_Incompressible_Mixed::get_strain_energy(
    const Tensor2_3D &F ) const
{
  ...
  const double PSI_fi1 = 0.5 * fk1 * fE1 * fE1;
  const double PSI_fi2 = 0.5 * fk1 * fE2 * fE2;

  return PSI_iso + PSI_fi1 + PSI_fi2;
}

Vector_3 MaterialModel_GOH11_Incompressible_Mixed::get_fibre_dir( const int &dir ) const
{
  if(dir == 0)
    return a1;
  else if(dir == 1)
  ...
  {
    SYS_T::print_fatal("Error: MaterialModel_GOH11_Incompressible_Mixed, wrong fibre direction. \n");
    return Vector_3();
  }
}

void MaterialModel_GOH11_Incompressible_Mixed::update_fibre_dir( const Vector_3 &basis_r,
