...
{
  // Now read the lpn input file for num_ebc and coronary model parameters (Ra, Ca, Ra_micro, Cim, Rv, Pd and Pim)
      ...
      || bc_type.compare("coronary") == 0 )
  {
    ...
    Rv.resize( num_ebc ); Pd.resize( num_ebc );
    alpha_Pim.resize(num_ebc);Q0.resize(num_ebc);
    ...
    dPimdt_k1.resize(num_ebc);dPimdt_k2.resize(num_ebc);
  ...
  // Read files for each ebc to set the values of Ra, Ca, Ra_micro, Cim,Rv,Pd Pim and alpha_Pim
  int counter = 0;
  int data_size=1;
  while( std::getline(reader, sline) )
  {
