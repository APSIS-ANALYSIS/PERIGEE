...
MaterialModel_GOH14_ST91_Degradation::MaterialModel_GOH14_ST91_Degradation(
    ...
    const double &in_fkd )
: rho0( in_rho ), E0(in_E), E(in_E), nu(in_nu),
  lambda0( nu * E / ((1+nu) * (1-2.0*nu)) ),
  lambda(nu * E / ((1+nu) * (1-2.0*nu))),
  mu0( E/(2.0+2.0*nu) ),
  mu( E/(2.0+2.0*nu) ), 
  kappa0( lambda + 2.0 * mu / 3.0 ),
  ...
  fk10(in_fk1), fk1(in_fk1), fk2(in_fk2), fkd(in_fkd)
{
    ...
    const char * const &fname )
{
  ...
  SYS_T::print_fatal_if( h5r->read_string("/", "model_name") != get_model_name(),
     "Error: MaterialModel_GOH14_ST91_Degradation constructor does not match h5 file.\n" );

  rho0   = h5r -> read_doubleScalar("/", "rho0");
  E0     = h5r -> read_doubleScalar("/", "E0");
  E      = h5r -> read_doubleScalar("/", "E");
  nu     = h5r -> read_doubleScalar("/", "nu");
  lambda0= h5r -> read_doubleScalar("/", "lambda0");
  lambda = h5r -> read_doubleScalar("/", "lambda");
  mu0    = h5r -> read_doubleScalar("/", "mu0");
  mu     = h5r -> read_doubleScalar("/", "mu");
  kappa0 = h5r -> read_doubleScalar("/", "kappa0");
  ...
  fk10   = h5r -> read_doubleScalar("/", "fk10");
...
void MaterialModel_GOH14_ST91_Degradation::print_info() const
{
  SYS_T::commPrint("\t  MaterialModel_GOH14_ST91_Degradation: \n");
...
void MaterialModel_GOH14_ST91_Degradation::write_hdf5( const char * const &fname ) const
{
  if( SYS_T::get_MPI_rank() == 0 )
  {
    ...
    h5w -> write_doubleScalar("E0", E0);
    h5w -> write_doubleScalar("E", E);
    h5w -> write_doubleScalar("nu", nu);
    h5w -> write_doubleScalar("lambda0", lambda0);
    h5w -> write_doubleScalar("lambda", lambda);
    h5w -> write_doubleScalar("mu0", mu0);
    h5w -> write_doubleScalar("mu", mu);
    h5w -> write_doubleScalar("kappa0", kappa0);
    ...
    h5w -> write_doubleScalar("fk10", fk10);
...
void MaterialModel_GOH14_ST91_Degradation::get_PK( 
...
void MaterialModel_GOH14_ST91_Degradation::get_PK_Stiffness( 
...
double MaterialModel_GOH14_ST91_Degradation::get_strain_energy(const Tensor2_3D &F ) const
{
  Tensor2_3D C; C.MatMultTransposeLeft(F);
  const double trC = C.tr();
  const double detFm0d67 = std::pow(F.det(), - 2.0 / 3.0);
...
Vector_3 MaterialModel_GOH14_ST91_Degradation::get_fibre_dir( const int &dir ) const
{
  if(dir == 0)
  {
    return a1;
  ...
  {
    SYS_T::print_fatal("Error: MaterialModel_GOH14_ST91_Degradation, wrong fibre direction. \n");
    return Vector_3();
  }
}

double MaterialModel_GOH14_ST91_Degradation::get_rho( const double &p ) const
{
  const double pk = p / kappa;
  return rho0 * (std::pow(pk*pk+1.0, 0.5) + pk);
}

double MaterialModel_GOH14_ST91_Degradation::get_drho_dp( const double &p ) const
{
  const double pk = p / kappa;
  return (rho0 / kappa) * (1 + pk * std::pow(pk*pk+1.0, -0.5));
}

double MaterialModel_GOH14_ST91_Degradation::get_beta( const double &p ) const
{
  const double pk = p / kappa;
  return 1.0 / (kappa * std::pow(pk*pk+1.0, 0.5));
}

double MaterialModel_GOH14_ST91_Degradation::get_dbeta_dp( const double &p ) const
{
  const double pk = p / kappa;
...
void MaterialModel_GOH14_ST91_Degradation::update_fibre_dir( const Vector_3 &basis_r,
