#include "FEAElement_BS_3D_der2.hpp"

FEAElement_BS_3D_der2::FEAElement_BS_3D_der2( const int &in_sdeg, 
    const int &in_tdeg, const int &in_udeg,
    const int &in_nquas, const int &in_nquat, const int &in_nquau )
{
  sdeg = in_sdeg;
  tdeg = in_tdeg;
  udeg = in_udeg;
...
FEAElement_BS_3D_der2::~FEAElement_BS_3D_der2()
{
  clearBasisCache();
}


void FEAElement_BS_3D_der2::clearBasisCache()
{
  delete [] R; R = NULL;
  delete [] Jac; Jac = NULL;
  delete [] dRr; dRr = NULL;
...
void FEAElement_BS_3D_der2::resize_container()
{
  clearBasisCache();
  R = new double [10 * rlength];
  Jac = new double [19 * numQuapts];
...
void FEAElement_BS_3D_der2::reset_degree( const int &new_sdeg, 
    const int &new_tdeg, const int &new_udeg )
{
  sdeg = new_sdeg;
  tdeg = new_tdeg;
  udeg = new_udeg;
...
void FEAElement_BS_3D_der2::reset_numQua( const int &new_squa, const int &new_tqua,
    const int &new_uqua )
{
  num_qua_s = new_squa;
  num_qua_t = new_tqua;
...
void FEAElement_BS_3D_der2::print() const
{
  SYS_T::commPrint("BS_3D_der2 : ");
  SYS_T::commPrint("Three-dimensional B-Spline shape function with 2nd derivatives. \n");
  PetscPrintf(PETSC_COMM_WORLD, "elemType: %d. \n", get_Type());
  SYS_T::commPrint("Note: Jacobian and inverse Jacobian are evaluated. \n");
}


double FEAElement_BS_3D_der2::get_memory_usage() const
{
  double double_size = rlength * 10 + numQuapts * 19 + nLocBas * 9 + 3 * (sdp1+tdp1+udp1);
  double int_size = 11;
  return double_size * 8.0 + int_size * 4.0;
}


void FEAElement_BS_3D_der2::buildBasis( const double &hx, const double &hy, const double &hz,
  ...
  for( int qua_z = 0; qua_z < num_qua_u; ++qua_z )
  {
    ...
    for( int qua_y = 0; qua_y < num_qua_t; ++qua_y )
    {
      ...
      for( int qua_x = 0; qua_x < num_qua_s; ++qua_x )
      {
        ...
        counter += 1;
        BuildShape_atQua(counter, hx, hy, hz, ctrl_x, ctrl_y, ctrl_z);
      }
    }
  }
...
void FEAElement_BS_3D_der2::BuildShape_atQua( const int &quaindex,
    ...
    const double * const &ctrl_z )
{
  ...
  for(kk=0; kk<udp1; ++kk)
  {
    for(jj=0; jj<tdp1; ++jj)
    {
      ...
      {
        ll += 1;
        R[offset + ll] = Nns[ii] * Nnt[jj] * Nnu[kk];

        dRr[ll] = Nns[sdp1 + ii] * Nnt[jj] * Nnu[kk];

        dx_ds += ctrl_x[ll] * dRr[ll];
        dy_ds += ctrl_y[ll] * dRr[ll];
        dz_ds += ctrl_z[ll] * dRr[ll];

        dRr[nLocBas + ll] = Nns[ii] * Nnt[tdp1+jj] * Nnu[kk];

        dx_dt += ctrl_x[ll] * dRr[nLocBas + ll];
        dy_dt += ctrl_y[ll] * dRr[nLocBas + ll];
        dz_dt += ctrl_z[ll] * dRr[nLocBas + ll];

        dRr[2*nLocBas + ll] = Nns[ii] * Nnt[jj] * Nnu[udp1 + kk];

        dx_du += ctrl_x[ll] * dRr[2*nLocBas + ll];
        dy_du += ctrl_y[ll] * dRr[2*nLocBas + ll]; 
        dz_du += ctrl_z[ll] * dRr[2*nLocBas + ll];

        dRr[3*nLocBas + ll] = Nns[2*sdp1 + ii] * Nnt[jj] * Nnu[kk];
        
        d2x_dss += ctrl_x[ll] * dRr[3*nLocBas + ll];
        d2y_dss += ctrl_y[ll] * dRr[3*nLocBas + ll];
        d2z_dss += ctrl_z[ll] * dRr[3*nLocBas + ll];

        dRr[4*nLocBas + ll] = Nns[ii] * Nnt[2*tdp1 + jj] * Nnu[kk];

        d2x_dtt += ctrl_x[ll] * dRr[4*nLocBas + ll];
        d2y_dtt += ctrl_y[ll] * dRr[4*nLocBas + ll];
        d2z_dtt += ctrl_z[ll] * dRr[4*nLocBas + ll];

        dRr[5*nLocBas + ll] = Nns[ii] * Nnt[jj] * Nnu[2*udp1+kk];

        d2x_duu += ctrl_x[ll] * dRr[5*nLocBas + ll];
        d2y_duu += ctrl_y[ll] * dRr[5*nLocBas + ll];
        d2z_duu += ctrl_z[ll] * dRr[5*nLocBas + ll];

        dRr[6*nLocBas + ll] = Nns[sdp1+ii] * Nnt[tdp1+jj] * Nnu[kk];

        d2x_dst += ctrl_x[ll] * dRr[6*nLocBas + ll];
        d2y_dst += ctrl_y[ll] * dRr[6*nLocBas + ll];
        d2z_dst += ctrl_z[ll] * dRr[6*nLocBas + ll];

        dRr[7*nLocBas + ll] = Nns[sdp1+ii] * Nnt[jj] * Nnu[udp1+kk];

        d2x_dsu += ctrl_x[ll] * dRr[7*nLocBas + ll];
        d2y_dsu += ctrl_y[ll] * dRr[7*nLocBas + ll];
        d2z_dsu += ctrl_z[ll] * dRr[7*nLocBas + ll];

        dRr[8*nLocBas + ll] = Nns[ii] * Nnt[tdp1+jj] * Nnu[udp1+kk];

        d2x_dtu += ctrl_x[ll] * dRr[8*nLocBas + ll];
        d2y_dtu += ctrl_y[ll] * dRr[8*nLocBas + ll];
        d2z_dtu += ctrl_z[ll] * dRr[8*nLocBas + ll];
      }
    }
  }

  double detJac_temp = dx_ds * dy_dt * dz_du + dx_dt * dy_du * dz_ds
  ...
  double inv_detJac = 1.0 / detJac_temp;
...
void FEAElement_BS_3D_der2::get_R( const int &quaindex, double * const &basis ) const
{
...
double FEAElement_BS_3D_der2::get_detJac(const int &quaindex) const
{
  return Jac[18*numQuapts + quaindex];
}


void FEAElement_BS_3D_der2::get_R_gradR( const int &quaindex, double * const &basis,
...
void FEAElement_BS_3D_der2::get_3D_R_dR_d2R( const int &quaindex, double * const &basis,
...
void FEAElement_BS_3D_der2::get_3D_R_gradR_LaplacianR( const int &quaindex,
...
void FEAElement_BS_3D_der2::get_Jacobian(const int &quaindex, double * const &jac_value) const
{
...
void FEAElement_BS_3D_der2::get_invJacobian(const int &quaindex, double * const &jac_value) const
{
...
void FEAElement_BS_3D_der2::get_3d_normal_bottom( const int &quaindex,
...
void FEAElement_BS_3D_der2::get_3d_normal_top( const int &quaindex,
    double &nx, double &ny, double &nz, double &surfaceArea ) const
{
  const int offset = 9 * quaindex;
...
void FEAElement_BS_3D_der2::get_3d_normal_left( const int &quaindex,
    double &nx, double &ny, double &nz, double &surfaceArea ) const
{
  const int offset = 9 * quaindex;
...
void FEAElement_BS_3D_der2::get_3d_normal_right( const int &quaindex,
    double &nx, double &ny, double &nz, double &surfaceArea ) const
{
  const int offset = 9 * quaindex;
...
void FEAElement_BS_3D_der2::get_3d_normal_front( const int &quaindex,
    double &nx, double &ny, double &nz, double &surfaceArea ) const
{
  const int offset = 9 * quaindex;
...
void FEAElement_BS_3D_der2::get_3d_normal_back( const int &quaindex,
    double &nx, double &ny, double &nz, double &surfaceArea ) const
{
  const int offset = 9 * quaindex;
