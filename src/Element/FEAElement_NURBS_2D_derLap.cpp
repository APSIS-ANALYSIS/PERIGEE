#include "FEAElement_NURBS_2D_derLap.hpp"

FEAElement_NURBS_2D_derLap::FEAElement_NURBS_2D_derLap( const int &in_eIndex,
...
FEAElement_NURBS_2D_derLap::~FEAElement_NURBS_2D_derLap()
{
  clearBasisCache();
}


void FEAElement_NURBS_2D_derLap::clearBasisCache()
{
  if(is_sNonzero)
  {
    delete [] R;        delete [] dR_dx;   delete [] dR_dy;
    delete [] d2R_dxx;  delete [] d2R_dyy;
    delete [] detJac;
  }
  R = NULL; dR_dx = NULL; dR_dy = NULL; detJac = NULL;
  d2R_dxx = NULL; d2R_dyy = NULL;
...
void FEAElement_NURBS_2D_derLap::buildBasis( const int &ssdp1, const int &ttdp1,
...
void FEAElement_NURBS_2D_derLap::get_R( const int &quaindex, 
...
void FEAElement_NURBS_2D_derLap::get_R_gradR(const int &quaindex,
...
void FEAElement_NURBS_2D_derLap::get_2D_R_gradR_LaplacianR( 
...
void FEAElement_NURBS_2D_derLap::print() const
{
  SYS_T::commPrint("NURBS_2D_derLap: ");
  SYS_T::commPrint("2-Dimensional NURBS shape function with 1st and Lap derivatives. \n");
  PetscPrintf(PETSC_COMM_WORLD, "elemType: %d. \n", get_Type());
  SYS_T::commPrint("Note: Jacobian matrix is not evaluated. \n");
}

double FEAElement_NURBS_2D_derLap::get_memory_usage() const
{
  unsigned int double_size = 0; unsigned int int_size = 3;
  double_size = nLocBas * numQuapts * 5 + numQuapts;
  return (double) double_size * 8.0 + (double) int_size * 4.0;
}


void FEAElement_NURBS_2D_derLap::BuildShape_atQua(
