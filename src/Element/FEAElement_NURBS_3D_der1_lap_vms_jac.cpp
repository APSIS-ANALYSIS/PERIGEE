...
FEAElement_NURBS_3D_der1_lap_vms_jac::FEAElement_NURBS_3D_der1_lap_vms_jac(
    const int &in_eIndex, const IALocal_meshSize * const &mSize, 
    ...
    const ALocal_IEN * const &locIEN)
{
  ...
  const int sDegree = Bs->get_degree();
  const int tDegree = Bt->get_degree();
  const int uDegree = Bu->get_degree();

  const int sDp1 = sDegree + 1; 
  const int tDp1 = tDegree + 1; 
  const int uDp1 = uDegree + 1;

  nLocBas = sDp1 * tDp1 * uDp1;

  const int num_qua_s = Bs->get_nQuapts();
  const int num_qua_t = Bt->get_nQuapts();
  const int num_qua_u = Bu->get_nQuapts();

  numQuapts = num_qua_s * num_qua_t * num_qua_u;

  is_sNonzero = true;
  const double refSize = mSize->get_hx(elem_index) * mSize->get_hy(elem_index) * 
...
FEAElement_NURBS_3D_der1_lap_vms_jac::FEAElement_NURBS_3D_der1_lap_vms_jac( 
    const int &in_eIndex,
    const double &inhx, const double &inhy, const double &inhz,
    const BernsteinBasis_Array * const &Bs,
    const BernsteinBasis_Array * const &Bt,
    const BernsteinBasis_Array * const &Bu,
    const double * const &inctrl_x,
    const double * const &inctrl_y,
    const double * const &inctrl_z,
    const double * const &inctrl_w,
    const double * const &inext_x,
    const double * const &inext_y,
    const double * const &inext_z )
{
  // Assign values to single data, initialize quadrature arrays.
  elem_index = in_eIndex;
  ...
  is_sNonzero = true;
  const double refSize = inhx * inhy * inhz;
  ...
  if(is_sNonzero == true)
  {
    buildBasis( inhx, inhy, inhz, Bs, Bt, Bu, inctrl_x, inctrl_y, inctrl_z,
        inctrl_w, inext_x, inext_y, inext_z );
  }
}
...
void FEAElement_NURBS_3D_der1_lap_vms_jac::clearBasisCache()
{
  if( is_sNonzero )
  {
    delete [] R; 
    ...
    delete [] detJac;
    delete [] ds_dx_qua; delete [] dx_ds_qua;
  }
  R = NULL; dR_dx = NULL; dR_dy = NULL; dR_dz = NULL; detJac = NULL;
  d2R_dxx = NULL; d2R_dyy = NULL; d2R_dzz = NULL;
  ds_dx_qua = NULL; dx_ds_qua = NULL;
}

void FEAElement_NURBS_3D_der1_lap_vms_jac::buildBasis(
    ...
    const ALocal_IEN * const &locIEN )
{
  // Update the element's total number of quadrature points for re-evaluation
  numQuapts = num_qs * num_qt * num_qu;
  
  // Allocate memory
  const int basis_size = nLocBas * numQuapts;
  ...
  ds_dx_qua = new double [9*numQuapts];
  dx_ds_qua = new double [9*numQuapts];

  // Assign necessary data
  const double hx = mSize->get_hx(elem_index);
  const double hy = mSize->get_hy(elem_index);
  const double hz = mSize->get_hz(elem_index);
    ...
    for(int qua_y = 0; qua_y<num_qt; ++qua_y)
    {
      ...
      for(int qua_x = 0; qua_x<num_qs; ++qua_x)
      {
        ...
        BuildShape_atQua( ssdp1, ttdp1, uudp1, counter, qua_x, qua_y, qua_z,
            hx, hy, hz, invhx, invhy, invhz, ctrl_x, ctrl_y, ctrl_z, ctrl_w, 
...
{
  // Obtain the degree from the Bernstein polynomial object
  const int ssdp1 = Bs->get_degree() + 1;
  const int ttdp1 = Bt->get_degree() + 1;
  const int uudp1 = Bu->get_degree() + 1;

  // Obtain the number of quadrature points from the Bernstein polynomial object
  const int num_qs = Bs->get_nQuapts();
  const int num_qt = Bt->get_nQuapts();
  const int num_qu = Bu->get_nQuapts();
...
void FEAElement_NURBS_3D_der1_lap_vms_jac::get_R(const int &quaindex, 
...
void FEAElement_NURBS_3D_der1_lap_vms_jac::get_gradR(const int &quaindex, 
...
void FEAElement_NURBS_3D_der1_lap_vms_jac::get_3D_R_gradR_LaplacianR(
...
void FEAElement_NURBS_3D_der1_lap_vms_jac::get_Jacobian( const int &quaindex, 
    double * const &jac_value ) const
{
  const int qstart = 9 * quaindex;
  for(int ii=0; ii<9; ++ii)
    jac_value[ii] = dx_ds_qua[qstart + ii];
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::get_invJacobian(const int &quaindex,
    double * const &dxi_dx) const
{
  const int qstart = 9 * quaindex;
  for(int ii=0; ii<9; ++ii)
    dxi_dx[ii] = ds_dx_qua[qstart + ii];
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::get_3d_normal_bottom(
    const int &quaindex, double &nx, double &ny, double &nz, double &surf_area ) const
{
  // n = r_t x r_s / | r_t x r_s |
  const int qstart = 9 * quaindex; 
  nx = dx_ds_qua[qstart+4] * dx_ds_qua[qstart+6] - dx_ds_qua[qstart+7] * dx_ds_qua[qstart+3];
  ny = dx_ds_qua[qstart+7] * dx_ds_qua[qstart+0] - dx_ds_qua[qstart+1] * dx_ds_qua[qstart+6];
  nz = dx_ds_qua[qstart+1] * dx_ds_qua[qstart+3] - dx_ds_qua[qstart+4] * dx_ds_qua[qstart+0];
  
  normalize_3d_vector(nx, ny, nz, surf_area);
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::get_3d_normal_top(
    const int &quaindex, double &nx, double &ny, double &nz, double &surf_area ) const
{
  // n = r_s x r_t / |r_s x r_t|
  const int qstart = 9 * quaindex;
  nx = dx_ds_qua[qstart+3] * dx_ds_qua[qstart+7] - dx_ds_qua[qstart+6] * dx_ds_qua[qstart+4];
  ny = dx_ds_qua[qstart+6] * dx_ds_qua[qstart+1] - dx_ds_qua[qstart+0] * dx_ds_qua[qstart+7];
  nz = dx_ds_qua[qstart+0] * dx_ds_qua[qstart+4] - dx_ds_qua[qstart+1] * dx_ds_qua[qstart+3];
  
  normalize_3d_vector(nx, ny, nz, surf_area);
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::get_3d_normal_left(
    const int &quaindex, double &nx, double &ny, double &nz, double &surf_area ) const
{
  // n = r_s x r_u / |r_s x r_u|
  const int qstart = 9 * quaindex;
  nx = dx_ds_qua[qstart+3] * dx_ds_qua[qstart+8] - dx_ds_qua[qstart+6] * dx_ds_qua[qstart+5];
  ny = dx_ds_qua[qstart+6] * dx_ds_qua[qstart+2] - dx_ds_qua[qstart+0] * dx_ds_qua[qstart+8];
  nz = dx_ds_qua[qstart+0] * dx_ds_qua[qstart+5] - dx_ds_qua[qstart+2] * dx_ds_qua[qstart+3];
  
  normalize_3d_vector(nx, ny, nz, surf_area);
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::get_3d_normal_right(
    const int &quaindex, double &nx, double &ny, double &nz, double &surf_area ) const
{
  // n = r_u x r_s / |r_u x r_s|
  const int qstart = 9 * quaindex;
  nx = dx_ds_qua[qstart+5] * dx_ds_qua[qstart+6] - dx_ds_qua[qstart+8] * dx_ds_qua[qstart+3];
  ny = dx_ds_qua[qstart+8] * dx_ds_qua[qstart+0] - dx_ds_qua[qstart+2] * dx_ds_qua[qstart+6];
  nz = dx_ds_qua[qstart+2] * dx_ds_qua[qstart+3] - dx_ds_qua[qstart+5] * dx_ds_qua[qstart+0];
  
  normalize_3d_vector(nx, ny, nz, surf_area);
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::get_3d_normal_front(
    const int &quaindex, double &nx, double &ny, double &nz, double &surf_area ) const
{
  // n = r_t x r_u / |r_t x r_u|
  const int qstart = 9 * quaindex;
  nx = dx_ds_qua[qstart+4] * dx_ds_qua[qstart+8] - dx_ds_qua[qstart+7] * dx_ds_qua[qstart+5];
  ny = dx_ds_qua[qstart+7] * dx_ds_qua[qstart+2] - dx_ds_qua[qstart+1] * dx_ds_qua[qstart+8];
  nz = dx_ds_qua[qstart+1] * dx_ds_qua[qstart+5] - dx_ds_qua[qstart+4] * dx_ds_qua[qstart+2];
  
  normalize_3d_vector(nx, ny, nz, surf_area);
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::get_3d_normal_back(
    const int &quaindex, double &nx, double &ny, double &nz, double &surf_area ) const
{
  // r_u x r_t / |r_u x r_t|
  const int qstart = 9 * quaindex;
  nx = dx_ds_qua[qstart+5] * dx_ds_qua[qstart+7] - dx_ds_qua[qstart+8] * dx_ds_qua[qstart+4];
  ny = dx_ds_qua[qstart+8] * dx_ds_qua[qstart+1] - dx_ds_qua[qstart+2] * dx_ds_qua[qstart+7];
  nz = dx_ds_qua[qstart+2] * dx_ds_qua[qstart+4] - dx_ds_qua[qstart+5] * dx_ds_qua[qstart+1];
  
  normalize_3d_vector(nx, ny, nz, surf_area);
}


void FEAElement_NURBS_3D_der1_lap_vms_jac::print() const
{
  SYS_T::commPrint("NURBS_3D_der1_lap_vms_jac: ");
  SYS_T::commPrint("3-Dimensional NURBS shape function with 1st order derivatives. \n");
  SYS_T::commPrint("and the Laplacian operator. \n");
  SYS_T::commPrint("      The 9 components of dxi_dx are evaluated at qua points. \n");
  SYS_T::commPrint("      The 9 components of dx_dxi are evaluated at qua points. \n");
}

double FEAElement_NURBS_3D_der1_lap_vms_jac::get_memory_usage() const
{
  unsigned int double_size = 0; unsigned int int_size = 3;
  double_size = nLocBas * numQuapts * 7 + numQuapts * 19;

  return (double) double_size * 8.0 + (double) int_size * 4.0;
}

void FEAElement_NURBS_3D_der1_lap_vms_jac::BuildShape_atQua(
    ...
    )
{
  ...
  // Now cache the dx_ds matrix in dx_ds_qua array
  const int qstart = 9 * quaindex;
  dx_ds_qua[qstart + 0] = m0;
  dx_ds_qua[qstart + 1] = m1;
  dx_ds_qua[qstart + 2] = m2;
  dx_ds_qua[qstart + 3] = m3;
  dx_ds_qua[qstart + 4] = m4;
  dx_ds_qua[qstart + 5] = m5;
  dx_ds_qua[qstart + 6] = m6;
  dx_ds_qua[qstart + 7] = m7;
  dx_ds_qua[qstart + 8] = m8;

  // Jacobian invertion get dxi/dx, dxi/dy, dxi/dz; deta/dx, deta/dy,
  // deta/dz; dzeta/dx, dzeta/dy, dzeta/dz at this quad point.
  const double inv_detJactmp = 1.0 / detJac_temp;
  const double ds_dx = (m4*m8 - m5*m7) * inv_detJactmp;
  const double ds_dy = (m2*m7 - m1*m8) * inv_detJactmp;
  const double ds_dz = (m1*m5 - m2*m4) * inv_detJactmp;
  const double dt_dx = (m5*m6 - m3*m8) * inv_detJactmp;
  const double dt_dy = (m0*m8 - m2*m6) * inv_detJactmp;
  const double dt_dz = (m2*m3 - m0*m5) * inv_detJactmp;
  const double du_dx = (m3*m7 - m4*m6) * inv_detJactmp;
  const double du_dy = (m1*m6 - m0*m7) * inv_detJactmp;
  const double du_dz = (m0*m4 - m1*m3) * inv_detJactmp;

  // Now savethe inversion of Jacobian in the nine allocated arrays
  ds_dx_qua[qstart + 0] = ds_dx * invhx;
  ds_dx_qua[qstart + 1] = ds_dy * invhx;
  ds_dx_qua[qstart + 2] = ds_dz * invhx;
  ds_dx_qua[qstart + 3] = dt_dx * invhy;
  ds_dx_qua[qstart + 4] = dt_dy * invhy;
  ds_dx_qua[qstart + 5] = dt_dz * invhy;
  ds_dx_qua[qstart + 6] = du_dx * invhz;
  ds_dx_qua[qstart + 7] = du_dy * invhz;
  ds_dx_qua[qstart + 8] = du_dz * invhz;
