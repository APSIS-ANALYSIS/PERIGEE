#include "FEAElement_NURBS_3D_der1_wJac_v3.hpp"

FEAElement_NURBS_3D_der1_wJac_v3::FEAElement_NURBS_3D_der1_wJac_v3(const int &in_eIndex,
...
FEAElement_NURBS_3D_der1_wJac_v3::~FEAElement_NURBS_3D_der1_wJac_v3()
{
  clearBasisCache();
}

void FEAElement_NURBS_3D_der1_wJac_v3::clearBasisCache()
{
  if(is_sNonzero)
  {
    delete [] R; 
...
void FEAElement_NURBS_3D_der1_wJac_v3::buildBasis(
...
void FEAElement_NURBS_3D_der1_wJac_v3::get_R(const int &quaindex, 
...
void FEAElement_NURBS_3D_der1_wJac_v3::get_gradR(const int &quaindex, 
...
double FEAElement_NURBS_3D_der1_wJac_v3::get_detJac( const int &quaindex ) const
{
  return detJac[quaindex];
}

void FEAElement_NURBS_3D_der1_wJac_v3::get_Jacobian( const int &quaindex,
    double * const &jac_value ) const
{
  for(int ii=0; ii<9; ++ii)
    jac_value[ii] = vecJac[quaindex*9 + ii];
}

void FEAElement_NURBS_3D_der1_wJac_v3::print() const
{
  SYS_T::commPrint("NURBS_3D_der1_wJac_v3: ");
  SYS_T::commPrint("3-Dimensional NURBS shape function with 1st order derivatives. \n");
  SYS_T::commPrint("Note: Jacobian matrix is evaluated. \n");
}

double FEAElement_NURBS_3D_der1_wJac_v3::get_memory_usage() const
{
  unsigned int double_size = 0; unsigned int int_size = 3;
  double_size = nLocBas * numQuapts * 4 + numQuapts * 10;

  return (double) double_size * 8.0 + (double) int_size * 4.0;
}

void FEAElement_NURBS_3D_der1_wJac_v3::BuildShape_atQua(
    ...
    )
{
  ...
  vecJac[9*quaindex + 0] = m0; 
  vecJac[9*quaindex + 1] = m1; 
  vecJac[9*quaindex + 2] = m2; 
  vecJac[9*quaindex + 3] = m3; 
  vecJac[9*quaindex + 4] = m4; 
  vecJac[9*quaindex + 5] = m5; 
  vecJac[9*quaindex + 6] = m6; 
  vecJac[9*quaindex + 7] = m7; 
  vecJac[9*quaindex + 8] = m8; 
