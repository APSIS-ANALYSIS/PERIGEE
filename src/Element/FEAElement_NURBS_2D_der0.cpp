...
FEAElement_NURBS_2D_der0::FEAElement_NURBS_2D_der0( const int &in_eIndex,
...
FEAElement_NURBS_2D_der0::~FEAElement_NURBS_2D_der0()
{
  clearBasisCache();
}


void FEAElement_NURBS_2D_der0::clearBasisCache()
{
  if(is_sNonzero)
  {
    delete [] R;
...
void FEAElement_NURBS_2D_der0::buildBasis( const int &ssdp1, const int &ttdp1,
...
void FEAElement_NURBS_2D_der0::get_R( const int &quaindex, double * const &basis ) const
{
  for(int ii=0; ii<nLocBas; ++ii)
...
double FEAElement_NURBS_2D_der0::get_detJac(const int &quaindex) const
{
  return detJac[quaindex];
}


void FEAElement_NURBS_2D_der0::print() const
{
  SYS_T::commPrint("NURBS_2D_der0: ");
  SYS_T::commPrint("2-Dimensional NURBS shape function with no derivatives. \n");
  PetscPrintf(PETSC_COMM_WORLD, "elemType: %d. \n", get_Type());
  SYS_T::commPrint("Note: Jacobian matrix is not evaluated. \n");
}


double FEAElement_NURBS_2D_der0::get_memory_usage() const
{
  unsigned int double_size = 0; unsigned int int_size = 3;
  double_size = nLocBas * numQuapts + numQuapts;
  return (double) double_size * 8.0 + (double) int_size * 4.0;
}


void FEAElement_NURBS_2D_der0::BuildShape_atQua(
