    ...
    const FEANode * const &fNode_ptr )
{
  ...
  // value[0] = u  value[1] = v  value[2] = w  value[3] = p
  ...
  {
    // vector locations for the [u,v,w,p] at the local node ii
  ...
  // Assembly the parallel vector object
  VecAssemblyBegin(solution);
  VecAssemblyEnd(solution);

  // Update values in ghost regions
  GhostUpdate();
    ...
    const FEANode * const &fNode_ptr )
{
  ...
  for(int ii=0; ii<nlocalnode; ++ii)
  {
    ...
    double dist0 = abs(n0[0]*(x-x0[0]) + n0[1]*(y-x0[1]) + n0[2]*(z-x0[2]));
    double dist1 = abs(n1[0]*(x-x1[0]) + n1[1]*(y-x1[1]) + n1[2]*(z-x1[2]));
    double dist2 = abs(n2[0]*(x-x2[0]) + n2[1]*(y-x2[1]) + n2[2]*(z-x2[2]));
  ...
  SYS_T::commPrint("              p = 0.5 on patch 2 bot. \n");
