...
PLinear_Solver_2x2Block_BIPN_SIMPLE::PLinear_Solver_2x2Block_BIPN_SIMPLE( 
    const double &rtol0,
  ...
  bipn_tol( b_rtol ), bipn_maxits( b_maxits ), ainv_case( in_ainv_case )
{
  ...
  // Allocate Schur complement
  MatCreateSchurComplement( gAssem_ptr->K_11, gAssem_ptr->K_11,
      gAssem_ptr->K_10, gAssem_ptr->K_01, gAssem_ptr->K_00, &S);

  // Set the inv type, diagonal or lumped
  if( ainv_case == 0 ) MatSchurComplementSetAinvType(S, MAT_SCHUR_COMPLEMENT_AINV_DIAG);
  else if( ainv_case == 1) MatSchurComplementSetAinvType(S, MAT_SCHUR_COMPLEMENT_AINV_LUMP);
  else SYS_T::print_fatal("PLinear_Solver_2x2Block_BIPN_SIMPLE: ainv_case is wrong. \n");

  // Generate Sp
  MatDuplicate( gAssem_ptr->K_00 , MAT_COPY_VALUES, &Sp );
  MatCreateSchurComplementPmat( gAssem_ptr->K_11,
            gAssem_ptr->K_10, gAssem_ptr->K_01, gAssem_ptr->K_00,
            MAT_SCHUR_COMPLEMENT_AINV_DIAG, MAT_INITIAL_MATRIX, &Sp);
}
...
void PLinear_Solver_2x2Block_BIPN_SIMPLE::SetLHS_RHS( 
    IPGAssem_2x2Block const * const &gAssem_ptr )
{
  // Update the Schur complement object
  MatSchurComplementUpdateSubMatrices( S, gAssem_ptr->K_11, gAssem_ptr->K_11,
      gAssem_ptr->K_10, gAssem_ptr->K_01, gAssem_ptr->K_00 );

  // Generate Sp from S
  MatSchurComplementGetPmat( S, MAT_REUSE_MATRIX, &Sp );
  
  // Assign matrices to solvers
  solver_0 -> SetOperator( S, Sp );
  solver_1 -> SetOperator( gAssem_ptr->K_11 );
  
  // Setup the inner solver in Schur S
  KSP ksp_s; MatSchurComplementGetKSP(S, &ksp_s);
  PC pc_s; KSPGetPC(ksp_s, &pc_s); 
...
void PLinear_Solver_2x2Block_BIPN_SIMPLE::DestroyOperators()
{
  MatDestroy(&Sp);
  MatDestroy(&S);
}
...
void PLinear_Solver_2x2Block_BIPN_SIMPLE::Solve_RHS( 
    IPGAssem_2x2Block const * const &gAssem_ptr,
    PDNSolution * const &sol_0, PDNSolution * const &sol_1 )
{
  ...
  do
  {
    ...
    // I use std::abs to make this robust when the difference is close to zero.
