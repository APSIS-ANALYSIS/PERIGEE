...
PGAssem::PGAssem( const IPLocAssem * const &locassem_ptr,
...
void PGAssem::Init_petsc_32(const int &nonzero_per_row, const int &num_loc_row)
{
...
void PGAssem::Init_petsc_35(const int &nonzero_per_row, const int &num_loc_row)
{
  MatCreateAIJ(PETSC_COMM_WORLD, num_loc_row, num_loc_row, PETSC_DECIDE,
      PETSC_DECIDE, nonzero_per_row, PETSC_NULL, nonzero_per_row, PETSC_NULL, &K);
...
void PGAssem::Init_petsc_35(const int &dnz, const int &onz, const int &num_loc_row)
{
  MatCreateAIJ(PETSC_COMM_WORLD, num_loc_row, num_loc_row, PETSC_DECIDE,
...
void PGAssem::Init_petsc_35(const PetscInt * const &dnz,
...
void PGAssem::EssBC_KG(const IALocal_BC * const &bc_part, const int &field)
{
...
void PGAssem::EssBC_G(const IALocal_BC * const &bc_part, const int &field)
{
  int local_dir = bc_part->get_Num_LD(field);
  int local_sla = bc_part->get_Num_LP(field);
  int row;
...
// void PGAssem::GetLocal(const double * const &array, const int * const &IEN,
...
void PGAssem::Assem_nonzero_estimate(
    const ALocal_Elem * const &alelem_ptr,
...
void PGAssem::Get_dnz_onz( const int &nElem,
    const ALocal_IEN * const &lien_ptr,
    const APart_Node * const &node_ptr,
    const IALocal_BC * const &bc_part,
...
void PGAssem::Assem_mass_residual(
    const PDNSolution * const &sol_a,
    const ALocal_Elem * const &alelem_ptr,
    IPLocAssem * const &lassem_ptr,
    const ALocal_IEN * const &lien_ptr,
