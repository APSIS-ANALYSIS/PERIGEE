...
ALocal_BC_2D::ALocal_BC_2D(const HDF5_PartReader * const &h5reader)
{
  ...
  // check the front and back face element is zero in number.
  for(int ii=0; ii<dof; ++ii)
  {
    if( get_NumLE_Fro(ii) != 0  )
    {
      PetscPrintf(PETSC_COMM_WORLD, "Error: 2D Local BC has nonzero # front element. \n");
      MPI_Abort(PETSC_COMM_WORLD, 1);
    }
    if( get_NumLE_Bac(ii) != 0  )
    {
      PetscPrintf(PETSC_COMM_WORLD, "Error: 2D Local BC has nonzero # back element. \n");
      MPI_Abort(PETSC_COMM_WORLD, 1);
    }
  }
}

ALocal_BC_2D::~ALocal_BC_2D()
{}

int ALocal_BC_2D::get_LDN(const int &dof_index, const int &node) const
{
  int offset = 0;
...
int ALocal_BC_2D::get_LPSN(const int &dof_index, const int &node) const
{
  int offset = 0;
  for(int ii=0; ii<dof_index; ++ii)
    offset += Num_LP[ii];
...
int ALocal_BC_2D::get_LPMN(const int &dof_index, const int &node) const
{
  int offset = 0;
  for(int ii=0; ii<dof_index; ++ii)
    offset += Num_LP[ii];
...
int ALocal_BC_2D::get_LFront_Elem(const int &dof_index, const int &ii) const
{
  int offset = 0;
...
int ALocal_BC_2D::get_LBack_Elem(const int &dof_index, const int &ii) const
{
  int offset = 0;
  for(int jj=0; jj<dof_index; ++jj)
...
int ALocal_BC_2D::get_LLeft_Elem(const int &dof_index, const int &ii) const
{
  int offset = 0;
  for(int jj=0; jj<dof_index; ++jj)
...
int ALocal_BC_2D::get_LRight_Elem(const int &dof_index, const int &ii) const 
{
  int offset = 0;
  for(int jj=0; jj<dof_index; ++jj)
...
int ALocal_BC_2D::get_LTop_Elem(const int &dof_index, const int &ii) const
{
  int offset = 0;
  for(int jj=0; jj<dof_index; ++jj)
...
int ALocal_BC_2D::get_LBottom_Elem(const int &dof_index, const int &ii) const
{
  int offset = 0;
  for(int jj=0; jj<dof_index; ++jj)
...
void ALocal_BC_2D::print() const
{
  std::cout<<"ALocal_BC_2D: \n";
