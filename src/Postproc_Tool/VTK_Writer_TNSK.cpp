#include "VTK_Writer_TNSK.hpp"

VTK_Writer_TNSK::VTK_Writer_TNSK( const IAGlobal_Mesh_Info * const &gmesh_ptr )
{
  nLocBas = gmesh_ptr->get_nLocBas();
  
  // allocate R for the private function interpolateNURBS
  R = new double [nLocBas];
}

VTK_Writer_TNSK::~VTK_Writer_TNSK()
{
  delete [] R;
}

void VTK_Writer_TNSK::interpolateNURBS( const double * const &inputVal,
...
void VTK_Writer_TNSK::interpolateNURBS(const double * const &inputVal_1,
...
void VTK_Writer_TNSK::interpolatePts( 
...
void VTK_Writer_TNSK::interpolateData( const int &size, const int &ptoffset,
...
void VTK_Writer_TNSK::interpolateTNSKData( const int &ptoffset,
    const double * const &inputRho,
    const double * const &inputVel,
    const double * const &inputThe,
    const double * const &inputEnt,
    const FEAElement * const &elem,
    vtkDoubleArray * const &vtkDataRho,
    vtkDoubleArray * const &vtkDataVel,
    vtkDoubleArray * const &vtkDataThe,
    vtkDoubleArray * const &vtkDataEnt ) const
{
  int nqp = elem->get_numQuapts();

  double * compDataRho = new double [nLocBas];
  double * compDataVeU = new double [nLocBas];
  double * compDataVeV = new double [nLocBas];
  double * compDataVeW = new double [nLocBas];
  double * compDataThe = new double [nLocBas];
  double * compDataEnt = new double [nLocBas];

  double * outDataRho = new double [nqp];
  double * outDataVeU = new double [nqp];
  double * outDataVeV = new double [nqp];
  double * outDataVeW = new double [nqp];
  double * outDataThe = new double [nqp];
  double * outDataEnt = new double [nqp];

  for(int ii=0; ii<nLocBas; ++ii)
  {
    compDataRho[ii] = inputRho[ii];
    compDataVeU[ii] = inputVel[ii*3];
    compDataVeV[ii] = inputVel[ii*3+1];
    compDataVeW[ii] = inputVel[ii*3+2];
    compDataThe[ii] = inputThe[ii];
    compDataEnt[ii] = inputEnt[ii];
  }

  interpolateNURBS(compDataRho, elem, outDataRho);
  interpolateNURBS(compDataVeU, elem, outDataVeU);
  interpolateNURBS(compDataVeV, elem, outDataVeV);
  interpolateNURBS(compDataVeW, elem, outDataVeW);
  interpolateNURBS(compDataThe, elem, outDataThe);
  interpolateNURBS(compDataEnt, elem, outDataEnt);

  for(int ii=0; ii<nqp; ++ii)
  {
    outDataThe[ii] = -1.0 / outDataThe[ii];
    outDataVeU[ii] = outDataVeU[ii] * outDataThe[ii];
    outDataVeV[ii] = outDataVeV[ii] * outDataThe[ii];
    outDataVeW[ii] = outDataVeW[ii] * outDataThe[ii];
  }

  for(int ii=0; ii<nqp; ++ii)
  {
    vtkDataRho->InsertComponent(ptoffset+ii, 0, outDataRho[ii]);
    vtkDataVel->InsertComponent(ptoffset+ii, 0, outDataVeU[ii]);
    vtkDataVel->InsertComponent(ptoffset+ii, 1, outDataVeV[ii]);
    vtkDataVel->InsertComponent(ptoffset+ii, 2, outDataVeW[ii]);
    vtkDataThe->InsertComponent(ptoffset+ii, 0, outDataThe[ii]);
    vtkDataEnt->InsertComponent(ptoffset+ii, 0, outDataEnt[ii]);
  }

  delete [] compDataRho;  delete [] compDataVeU;
  delete [] compDataVeV;  delete [] compDataVeW;
  delete [] compDataThe;  delete [] compDataEnt;

  delete [] outDataRho;  delete [] outDataVeU;
  delete [] outDataVeV;  delete [] outDataVeW;
  delete [] outDataThe;  delete [] outDataEnt;

}


void VTK_Writer_TNSK::interpolateTNSKData_pre( const int &ptoffset,
    const double * const &inputRho,
    const double * const &inputVel,
    const double * const &inputThe,
    const double * const &inputEnt,
    ...
    vtkDoubleArray * const &vtkDataVel,
    vtkDoubleArray * const &vtkDataThe,
    vtkDoubleArray * const &vtkDataEnt,
    vtkDoubleArray * const &vtkDataPre ) const
{
  int nqp = elem->get_numQuapts();
  ...
  double * outDataThe = new double [nqp];
  double * outDataEnt = new double [nqp];
  double * outDataPre = new double [nqp];
  ...
  for(int ii=0; ii<nqp; ++ii)
  {
    ...
    outDataVeV[ii] = outDataVeV[ii] * outDataThe[ii];
    outDataVeW[ii] = outDataVeW[ii] * outDataThe[ii];
    outDataPre[ii] = 8.0 * outDataThe[ii] * outDataRho[ii]/(27.0 - 27.0 * outDataRho[ii])
      - outDataRho[ii] * outDataRho[ii];
  }
  ...
  for(int ii=0; ii<nqp; ++ii)
  {
    ...
    vtkDataThe->InsertComponent(ptoffset+ii, 0, outDataThe[ii]);
    vtkDataEnt->InsertComponent(ptoffset+ii, 0, outDataEnt[ii]);
    vtkDataPre->InsertComponent(ptoffset+ii, 0, outDataPre[ii]);
  }
...
void VTK_Writer_TNSK::setHexelem( const int &segs, const int &segt, 
...
void VTK_Writer_TNSK::setQuadelem( const int &segs, const int &segt, 
...
void VTK_Writer_TNSK::read_epart( const std::string &epart_file, const int &esize,
...
void VTK_Writer_TNSK::build3doutput(vtkUnstructuredGrid * const &gridData,
    ...
    const double * const * const &pointArrays ) const
{
  ...
  if(numDArrays != 5)
  {
    PetscPrintf(PETSC_COMM_WORLD, "Error: visualization preparation file is not ");
    PetscPrintf(PETSC_COMM_WORLD, "for TNSK file, which should have 5 vis data. \n ");
    MPI_Abort(PETSC_COMM_WORLD, 1);
  }
  ...
  for(int ee=0; ee<lelem_ptr->get_nlocalele(); ++ee)
  {
    ...
    if(elem_ptr->is_sizeNonzero())
    {
      ...
      std::vector<double> inputRho, inputVel, inputThe, inputEnt;
      inputRho.clear(); inputVel.clear(); inputThe.clear(); inputEnt.clear();

      for(int jj=0; jj<nLocBas; ++jj)
      {
        int pt_index = IEN_e[jj];
        
        inputRho.push_back(pointArrays[0][pt_index]);
        inputVel.push_back(pointArrays[1][pt_index*3]);
        inputVel.push_back(pointArrays[1][pt_index*3+1]);
        inputVel.push_back(pointArrays[1][pt_index*3+2]);
        inputThe.push_back(pointArrays[2][pt_index]);
        inputEnt.push_back(pointArrays[3][pt_index]);
      }

      interpolateTNSKData_pre(ptOffset, &inputRho[0], &inputVel[0], &inputThe[0],
          &inputEnt[0], elem_ptr, dataVecs[0], dataVecs[1],
          dataVecs[2], dataVecs[3], dataVecs[4]);
...
void VTK_Writer_TNSK::build2doutput(vtkUnstructuredGrid * const &gridData,
    const std::string &epart_file, const int nElem,
    const FEANode * const &fNode_ptr,
    const IALocal_meshSize * const &lmsize_ptr,
    const IAExtractor * const &ext_ptr,
...
void VTK_Writer_TNSK::writeOutput( const std::string &epart_file,
...
void VTK_Writer_TNSK::writeGridObject( vtkUnstructuredGrid * gridData,
...
void VTK_Writer_TNSK::writepvtuFile( vtkUnstructuredGrid * gridData,
    const std::string &baseName, const int &rank, const int &size,
    const bool &isXML) const
{
  if( isXML && rank == 0 && size > 1 )
  {
    ...
    if( !pvtuFile.is_open() )
    {
      cerr<<"ERROR: failed to open pvtu file "<<pName<<endl;
      MPI_Abort(PETSC_COMM_WORLD, 1);
    }
...
void VTK_Writer_TNSK::writepvdFile_Init( const std::string &pvdFName ) const
{  
  ...
  if( !pvdFile.is_open() )
  {
    cerr<<"ERROR: UNABLE TO OPEN THE PVD FILE : "<<pvdFName<<endl;
    MPI_Abort(PETSC_COMM_WORLD, 1);
  }
...
void VTK_Writer_TNSK::writepvdFile( const std::string &baseName,  
    ...
    const double &sol_time, const bool &isXML ) const
{
  if( rank == 0 && isXML == true )
  {
    ...
    if( !pvdFile.is_open() )
    {
      cerr<<"ERROR: FAIL TO OPEN THE PVD FILE "<<pvdname<<endl;
      MPI_Abort(PETSC_COMM_WORLD, 1);
    }
