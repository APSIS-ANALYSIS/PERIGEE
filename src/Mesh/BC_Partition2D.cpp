...
BC_Partition2D::BC_Partition2D( const IPart * const &part,
    ...
    std::vector<BoundaryCond2D *> const &bc_list )
{
  ...
  int llef_elem, lrig_elem, ltop_elem, lbot_elem;
  int temp_elem, elemlocindex;
  for(int i=0; i<dof; ++i)
  {
    // set front back zero for 2D geometry
    LFront_Elem.clear();
    LBack_Elem.clear();
    Num_LBCElem[6*i+0] = 0;
    Num_LBCElem[6*i+1] = 0;
...
BC_Partition2D::~BC_Partition2D()
{}

void BC_Partition2D::write_hdf5(const char * FileName) const
{
  ...
  status = H5Dwrite( setid_LID, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &LID[0] );
  status = H5Dclose( setid_LID );
  status = H5Sclose( dataspace_id_LID );
  ...
  if(LDN.size() > 0)
  {
    ...
    setid_LDN = H5Dcreate( group_id, "LDN", H5T_NATIVE_INT, dataspace_id_LDN,
        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
    status = H5Dwrite( setid_LDN, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &LDN[0] );
    status = H5Dclose( setid_LDN );
    status = H5Sclose( dataspace_id_LDN );
  }
  ...
  if(LPSN.size() > 0)
  {
    ...
    setid_LPSN = H5Dcreate( group_id, "LPSN", H5T_NATIVE_INT, dataspace_id_LPN,
        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
    status = H5Dwrite( setid_LPSN, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &LPSN[0] );
    setid_LPMN = H5Dcreate( group_id, "LPMN", H5T_NATIVE_INT, dataspace_id_LPN,
        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
    status = H5Dwrite( setid_LPMN, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &LPMN[0] );
    status = H5Dclose( setid_LPMN );
    status = H5Dclose( setid_LPSN );
    status = H5Sclose( dataspace_id_LPN );
  }
  ...
  setid_Num_LD = H5Dcreate( group_id, "Num_LD", H5T_NATIVE_INT, dataspace_id_Num_LD,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
  status = H5Dwrite( setid_Num_LD, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &Num_LD[0] );
  status = H5Dclose( setid_Num_LD );
  status = H5Sclose( dataspace_id_Num_LD ); 
  ...
  setid_Num_LP = H5Dcreate( group_id, "Num_LP", H5T_NATIVE_INT, dataspace_id_Num_LP,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
  status = H5Dwrite( setid_Num_LP, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, &Num_LP[0] );
  status = H5Dclose( setid_Num_LP );
  status = H5Sclose( dataspace_id_Num_LP );
  ...
  if( LFront_Elem.size() > 0 )
  {
    ...
    status = H5Dwrite( setid_LFront_Elem, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, 
        H5P_DEFAULT, &LFront_Elem[0] );
    status = H5Dclose( setid_LFront_Elem );
    status = H5Sclose( dataspace_id_LFront_Elem );
  }
  ...
  if( LBack_Elem.size() > 0 )
  {
    ...
    status = H5Dwrite( setid_LBack_Elem, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, 
        H5P_DEFAULT, &LBack_Elem[0] );
    status = H5Dclose( setid_LBack_Elem );
    status = H5Sclose( dataspace_id_LBack_Elem );
  }
  ...
  if( LLeft_Elem.size() > 0 )
  {
    ...
    status = H5Dwrite( setid_LLeft_Elem, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, 
        H5P_DEFAULT, &LLeft_Elem[0] );
    status = H5Dclose( setid_LLeft_Elem );
    status = H5Sclose( dataspace_id_LLeft_Elem );
  }
  ...
  if( LRight_Elem.size() > 0 )
  {
    ...
    status = H5Dwrite( setid_LRight_Elem, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, 
        H5P_DEFAULT, &LRight_Elem[0] );
    status = H5Dclose( setid_LRight_Elem );
    status = H5Sclose( dataspace_id_LRight_Elem );
  }
  ...
  if( LTop_Elem.size() > 0 )
  {
    ...
    status = H5Dwrite( setid_LTop_Elem, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, 
        H5P_DEFAULT, &LTop_Elem[0] );
    status = H5Dclose( setid_LTop_Elem );
    status = H5Sclose( dataspace_id_LTop_Elem );
  }
  ...
  if( LBottom_Elem.size() > 0 )
  {
    ...
    status = H5Dwrite( setid_LBottom_Elem, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, 
        H5P_DEFAULT, &LBottom_Elem[0] );
    status = H5Dclose( setid_LBottom_Elem );
    status = H5Sclose( dataspace_id_LBottom_Elem );
  }
  ...
  status = H5Dwrite( setid_Num_LBCElem, H5T_NATIVE_INT, H5S_ALL, H5S_ALL,
      H5P_DEFAULT, &Num_LBCElem[0] );
  status = H5Dclose( setid_Num_LBCElem );
  status = H5Sclose( dataspace_id_Num_LBCElem );
