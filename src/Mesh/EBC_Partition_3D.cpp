...
EBC_Partition_3D::EBC_Partition_3D( const IPart * const &part,
...
: cpu_rank(part->get_cpu_rank())
{
  ...
  unsigned int ltop_elem = 0;
  for(unsigned int jj=0; jj<ebc->get_num_top_elem(); ++jj)
  {
    temp_elem = ebc->get_top_elem(jj);
    elemlocindex = part->get_elemLocIndex(temp_elem);
    if(elemlocindex != -1)
    {
  ...
  Num_LBCElem[4] = ltop_elem; // top

  unsigned int lbot_elem = 0;
  for(unsigned int jj=0; jj<ebc->get_num_bottom_elem(); ++jj)
  {
    temp_elem = ebc->get_bottom_elem(jj);
    elemlocindex = part->get_elemLocIndex(temp_elem);
    if(elemlocindex != -1)
    {
  ...
  Num_LBCElem[5] = lbot_elem;

  VEC_T::shrink2fit(LFront_Elem);
  VEC_T::shrink2fit(LBack_Elem);
  VEC_T::shrink2fit(LLeft_Elem);
  VEC_T::shrink2fit(LRight_Elem);
  VEC_T::shrink2fit(LTop_Elem);
  VEC_T::shrink2fit(LBottom_Elem);
}

EBC_Partition_3D::~EBC_Partition_3D()
{
  ...
  //cout<<"--- EBC_Partition_3D deleted. \n";
}

void EBC_Partition_3D::write_hdf5(const char * FileName) const
{
  ...
  if(LTop_Elem.size() > 0)
    h5writer->write_intVector(group_id, "LTop_Elem", LTop_Elem);

  if(LBottom_Elem.size() > 0)
    h5writer->write_intVector(group_id, "LBottom_Elem", LBottom_Elem);
...
void EBC_Partition_3D::print_info() const
{
  std::cout<<"=========================================== \n";
  std::cout<<"EBC_Partition_3D : \n";
