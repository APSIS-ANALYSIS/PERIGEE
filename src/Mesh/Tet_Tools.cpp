    ...
    const double &num_shifted_elem )
{
  ...
  std::vector<int> gnode = VTK_T::read_int_PointData(file, "GlobalNodeID");
  std::vector<int> gelem = VTK_T::read_int_CellData(file, "GlobalElementID");
  
  for(int &node : gnode)
    node += num_shifted_node;

  for(int &elem : gelem)
    elem += num_shifted_elem;
