...
Part_NURBS_1Patch_2D_METIS::Part_NURBS_1Patch_2D_METIS( const IMesh * const &mesh,
    ...
    const bool isPrintInfo )
{
  ...
  // adopt basic global mesh info
  Get_global_meshinfo(mesh);

  // generate local partition info
  Generate_Partition(mesh, gpart, mnindex, IEN, ctrlPts, isPrintInfo);

  // store pointers to the seg_x(y,z)
...
Part_NURBS_1Patch_2D_METIS::~Part_NURBS_1Patch_2D_METIS()
{
...
void Part_NURBS_1Patch_2D_METIS::print_part_ele() const
{
...
void Part_NURBS_1Patch_2D_METIS::print_part_node() const
{
...
void Part_NURBS_1Patch_2D_METIS::print_part_ghost_node() const
{
...
void Part_NURBS_1Patch_2D_METIS::print_part_local_to_global() const
{
...
void Part_NURBS_1Patch_2D_METIS::print_part_LIEN() const
{
...
void Part_NURBS_1Patch_2D_METIS::print_part_loadbalance_edgecut() const
{
...
void Part_NURBS_1Patch_2D_METIS::print_part_bezier_ext_x(int elem) const
{
  int elem_x = elem % nElem_x;
...
void Part_NURBS_1Patch_2D_METIS::print_part_bezier_ext_y(int elem) const
{
  int elem_x, elem_y;
  elem_x = elem % nElem_x; 
  elem_y = (elem - elem_x) / nElem_x;
  std::vector<double> extractor;
...
void Part_NURBS_1Patch_2D_METIS::write(const char * inputFileName) const
{
...
void Part_NURBS_1Patch_2D_METIS::Get_global_meshinfo( const class IMesh * const &mesh )
{
  // inherit fundamental global mesh infos in the partition file
...
void Part_NURBS_1Patch_2D_METIS::Generate_Partition( const class IMesh * const &mesh,
    ...
    const bool isPrintInfo )
{
  ...
  // 2. reorder node_loc based on the mnindex ordering object
...
void Part_NURBS_1Patch_2D_METIS::Get_bezier_ext_x(const int &elem_x, std::vector<double> &ext_x) const
{
...
void Part_NURBS_1Patch_2D_METIS::Get_bezier_ext_y(const int &elem_y, std::vector<double> &ext_y) const
{
...
void Part_NURBS_1Patch_2D_METIS::write_local_elem( hid_t file_id ) const
{
  ...
  // h5writer->write_doubleScalar(group_id, "nlocalele_1", nlocalele );
  // h5writer->write_intVector(group_id, "elem_loc_1", elem_loc);
  // delete h5writer;

  H5Gclose( group_id );
}
  
void Part_NURBS_1Patch_2D_METIS::write_local_node( hid_t file_id ) const
{
...
void Part_NURBS_1Patch_2D_METIS::write_part_info( hid_t file_id ) const
{
...
void Part_NURBS_1Patch_2D_METIS::write_global_mesh_info( hid_t file_id ) const
{
  ...
  setid_degree = H5Dcreate( group_id, "degree", H5T_NATIVE_INT, dataspace_id_3,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
  vec_temp.clear();
  vec_temp.push_back(sDegree); vec_temp.push_back(tDegree);
  ...
  vec_temp2.clear();
  vec_temp2.push_back(hx_max); vec_temp2.push_back(hy_max);
  ...
  setid_hmin = H5Dcreate( group_id, "h_min", H5T_NATIVE_DOUBLE, dataspace_id_3,
      H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT );
  vec_temp2.clear();
  vec_temp2.push_back(hx_min); vec_temp2.push_back(hy_min);
...
void Part_NURBS_1Patch_2D_METIS::write_LIEN( hid_t file_id ) const
{
...
void Part_NURBS_1Patch_2D_METIS::write_ctrlPts_loc( hid_t file_id ) const
{
...
void Part_NURBS_1Patch_2D_METIS::write_extractor( hid_t file_id ) const
{
  hid_t group_id, dataspace_id_x, dataspace_id_y;
  hid_t setid_x, setid_y;
  hsize_t dim_x[2], dim_y[2];
  ...
  for(int e=0; e<nlocalele; ++e)
  {
    ...
    int e_global_x, e_global_y;

    e_global_x = e_global % nElem_x;
    e_global_y = (e_global - e_global_x) / nElem_x;

    std::vector<double> e_ext_x, e_ext_y;
  ...
  delete [] temp_ext_x; delete [] temp_ext_y;
...
void Part_NURBS_1Patch_2D_METIS::write_hxyz( hid_t file_id ) const
{
  HDF5_Writer * h5writer = new HDF5_Writer( file_id );
