...
NBC_Partition_moving_MF::NBC_Partition_moving_MF( const IPart * const &part,
    const Map_Node_Index * const &mnindex,
    const INodalBC * const &nbc,
    const std::vector< std::vector<int> > &grid2id )
: NBC_Partition_moving(part, mnindex, nbc)
{
  SYS_T::print_fatal_if( VEC_T::get_size(grid2id) != 3, "Error: NBC_Partition_moving_MF, the grid2id array size should be 3 in NBC_Partition_moving_MF.\n" );

  LDN_MF.resize(num_nbc);

  // The moving has 3 dofs
...
void NBC_Partition_moving_MF::write_hdf5( const std::string &FileName ) const
{
  // --------------------------------------------------------------------------
  // Call the base class writer to write the base class data
  NBC_Partition_moving::write_hdf5( FileName );
  // --------------------------------------------------------------------------
  ...
  hid_t g_id = H5Gopen(file_id, "/moving", H5P_DEFAULT);
