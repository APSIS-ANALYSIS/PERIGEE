...
PGAssem_Seg_FEM::PGAssem_Seg_FEM( IPLocAssem * const &locassem_ptr,
    ...
    ALocal_EBC const * const &part_ebc )
{
  ...
  for(int ebc_id=0; ebc_id < num_ebc; ++ebc_id){
    SYS_T::print_fatal_if(snLocBas != part_ebc->get_cell_nLocBas(ebc_id),
        "Error: in PGAssem_Seg_FEM, snLocBas has to be uniform. \n");
  }
  ...
  // Do a precise preallocation
  MatCreateAIJ(PETSC_COMM_WORLD, nlocrow, nlocrow, PETSC_DETERMINE,
      PETSC_DETERMINE, 0, &Kdnz[0], 0, &Konz[0], &K);
}


PGAssem_Seg_FEM::~PGAssem_Seg_FEM()
{
  VecDestroy(&G);
  MatDestroy(&K);
...
void PGAssem_Seg_FEM::Get_dnz_onz( const int &nlocnode,
...
void PGAssem_Seg_FEM::Assem_nonzero_estimate(
...
void PGAssem_Seg_FEM::Assem_mass_residual(
...
void PGAssem_Seg_FEM::Assem_residual(
    const PDNSolution * const &sol_a,
...
void PGAssem_Seg_FEM::Assem_tangent_residual(
    const PDNSolution * const &sol_a,
    const PDNSolution * const &sol_b,
    const double &curr_time,
    const double &dt,
...
void PGAssem_Seg_FEM::NatBC_G( const double &curr_time, const double &dt,
    IPLocAssem * const &lassem_ptr,
